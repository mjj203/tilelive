...
tilelive.auto = function(uri) {
    ...
    if (!tilelive.protocols[uri.protocol]) {
        [keyword, '@mapbox/' + keyword, 'tilelive-' + keyword, '@mapbox/tilelive-' + keyword].forEach(function(name) {
            try {
...
tilelive.validate = function(info) {
    ...
    for (var key in info) {
        ...
        case 'scheme':
            ...
            break;
        case 'minzoom':
            if (typeof val !== 'number' || val < 0 || val > 30 || Math.floor(val) !== val)
                return new Error('minzoom must be an integer between 0 and 30');
            break;
        case 'maxzoom':
            if (typeof val !== 'number' || val < 0 || val > 30 || Math.floor(val) !== val)
                return new Error('maxzoom must be an integer between 0 and 30');
            break;
        case 'name':
        case 'version':
        ...
        case 'center':
            ...
            if (val[2] < 0 || val[2] > 23 || Math.floor(val[2]) !== val[2])
                return new Error('center z value must be an integer between 0 and 30');
            break;
        case 'bounds':
...
tilelive.createReadStream = function(source, options) {
    ...
    if (options.type === 'put') throw new Error(options.type + ' is not a valid readable stream type');
